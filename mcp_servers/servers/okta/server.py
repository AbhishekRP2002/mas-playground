...
def create_server(enable_auth: bool = False) -> FastMCP:
    """Create and configure the Okta MCP server using FastMCP"""
    try:
        mcp = FastMCP(
        ...
        from mcp_servers.servers.okta.utils.okta_client import OktaMcpClient

        okta_client = OktaMcpClient()  # No immediate initialization

        logger.info("Registering Okta tools")
        from mcp_servers.servers.okta.tools.user_tools import register_user_tools
        from mcp_servers.servers.okta.tools.apps_tools import register_apps_tools
        from mcp_servers.servers.okta.tools.log_events_tools import register_log_events_tools
        from mcp_servers.servers.okta.tools.group_tools import register_group_tools
        from mcp_servers.servers.okta.tools.policy_network_tools import register_policy_tools
        from mcp_servers.servers.okta.tools.datetime_tools import register_datetime_tools
        from mcp_servers.servers.okta.tools.special_tools.access_analysis_tools import (
            register_access_analysis_tools,
        )
        from mcp_servers.servers.okta.tools.special_tools.suspicious_report_analysis import (
...
def run_with_stdio(server: FastMCP):
...
def run_with_http(server: FastMCP, host: str = "0.0.0.0", port: int = 3000):
...
def run_with_sse(server: FastMCP, host: str = "0.0.0.0", port: int = 3000):
